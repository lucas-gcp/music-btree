...
bTree *btCreateFromDisk(int order, int node_count, int root_disk, FILE *fp) {
    bTree *tree;                                // creates the "header" of the B-Tree
    if((tree = malloc(sizeof(bTree))) == NULL)  // allocate space for the new tree
        return NULL;

    btNode root = disk_read(root_disk, order, fp);        // creates the root of the new B-Tree
    root.pos_in_disk = root_disk;                         // give the root a position in the file

    tree->order = order;                        // give the tree it`s order
    tree->root = root;                          // give the tree it`s root
    tree->node_count = node_count;              // set the tree`s node count to 0
