...
auto create_directory(const std::string& path) -> bool {
    ...
    while (getline(path_stream, dir_path, '/')) {
        ...
        int status;
        #if defined(_WIN64)
            // std::wstring ws(current_path.begin(), current_path.end());
            // status = CreateDirectory(current_path.c_str(), NULL);
            status = mkdir(current_path.c_str());
        #elif defined(__unix__)
            status = mkdir(current_path.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        #endif
